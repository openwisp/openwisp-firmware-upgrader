...
class AbstractUpgradeOperation(UpgradeOptionsMixin, TimeStampedEditableModel):
    ...
    def log_line(self, line, save=True):
        ...
        try:
            ...
            # Publish to batch upgrade channel if this operation is part of a batch
            if self.batch:
                batch_publisher = BatchUpgradeProgressPublisher(self.batch.pk)
                # Calculate progress percentage from log content
                progress = self._calculate_progress_from_log()
                batch_publisher.publish_operation_progress(
                    str(self.pk), status_str, int(progress)
                )
    ...
    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        publisher = UpgradeProgressPublisher(self.pk)
        publisher.publish_progress({"type": "status", "status": str(self.status)})
        ...
        # Publish to batch upgrade channel if part of a batch
        if self.batch:
            batch_publisher = BatchUpgradeProgressPublisher(self.batch.pk)
            progress = self._calculate_progress_from_log()
            
            # Always publish progress update for existing operations
            batch_publisher.publish_operation_progress(
                str(self.pk), str(self.status), int(progress)
            )
            
            if self.status != "in-progress":
                self.batch.update()
            # Always publish batch progress for any operation status change
            self.batch._publish_batch_progress()
        return self
