...
def backfill_board_from_hardware_map(apps, schema_editor):
    ...
    for image_type, info in OPENWRT_FIRMWARE_IMAGE_MAP.items():
        ...
        if len(boards) == 1:
            ...
            if _write_multi_board_log(FirmwareImage, Build, image_type, list(boards)):
    ...
    if custom_images:
        ...
        for image_type, info in custom_images.items():
            ...
            if len(boards) == 1:
                ...
                if _write_multi_board_log(
                    FirmwareImage, Build, image_type, list(boards)
                ):
                    has_multi_board_images = True
