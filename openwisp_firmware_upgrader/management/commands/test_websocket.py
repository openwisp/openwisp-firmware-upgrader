...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'device_id',
            type=str,
            help='Device ID to test WebSocket publishing for'
        )
        parser.add_argument(
            '--operation-id',
            type=str,
            help='Operation ID (optional)',
            default=None
        )
    ...
    def handle(self, *args, **options):
        ...
        if hasattr(publisher, 'operation_group_name'):
            self.stdout.write(f"Publisher operation group name: {publisher.operation_group_name}")
        self.stdout.write(f"Expected consumer group name: firmware_upgrader.device-{device_id}")
