...
class TestRealTimeProgress(
    ...
    def test_individual_operation_progress_updates(self):
        ...
        # open individual upgrade page and ensure real time updates work there too
        links = self.find_elements(By.CSS_SELECTOR, ".results-container .device-link")
        ...
        with self.subTest("Ensure progress update is rendered correctly"):
            operation1.progress = 50
            operation1.log = "significant progress"
            operation1.save()
            progress_fill = self.find_element(
                By.CSS_SELECTOR, ".upgrade-status-container .upgrade-progress-fill"
            )
            ...
            self.assertIn("in progress", status_text)
            log = self.find_element(By.CSS_SELECTOR, ".field-log .readonly").text
            self.assertEqual(log, "significant progress")
            self._assert_no_js_errors()

        with self.subTest("Ensure abortion is rendered correctly"):
            operation1.progress = 0
            operation1.log = "Aborted for testing purposes."
            operation1.status = "aborted"
            operation1.save()
            progress_fill = self.find_element(
                By.CSS_SELECTOR,
            ...
            self.assertEqual(progress_text, [])
            status_field = self.find_element(By.CSS_SELECTOR, ".field-status .readonly")
            status_text = status_field.get_attribute("textContent").strip()
            self.assertIn("aborted", status_text)
            log = self.find_element(By.CSS_SELECTOR, ".field-log .readonly").text
            self.assertEqual(log, "Aborted for testing purposes.")
            self._assert_no_js_errors()

        with self.subTest("Ensure cancellation is rendered correctly"):
            operation1.log = "Cancelled for testing purposes."
            operation1.status = "cancelled"
            operation1.save()
            progress_fill = self.find_element(
                By.CSS_SELECTOR,
            ...
            status_field = self.find_element(By.CSS_SELECTOR, ".field-status .readonly")
            status_text = status_field.get_attribute("textContent").strip()
            self.assertIn("cancelled", status_text)
            log = self.find_element(By.CSS_SELECTOR, ".field-log .readonly").text
            self.assertEqual(log, "Cancelled for testing purposes.")
            self._assert_no_js_errors()

        with self.subTest("Ensure failure is rendered correctly"):
            operation1.log = "Failed because whatever."
            operation1.status = "failed"
            operation1.save()
            progress_fill = self.find_element(
                By.CSS_SELECTOR,
            ...
            status_field = self.find_element(By.CSS_SELECTOR, ".field-status .readonly")
            status_text = status_field.get_attribute("textContent").strip()
            self.assertIn("failed", status_text)
            log = self.find_element(By.CSS_SELECTOR, ".field-log .readonly").text
            self.assertEqual(log, "Failed because whatever.")
            self._assert_no_js_errors()

        with self.subTest("Ensure success is rendered correctly"):
            operation1.log = "Succeeded because you're very smart!"
            operation1.status = "success"
            operation1.save()
            progress_fill = self.find_element(
                By.CSS_SELECTOR,
            ...
            status_field = self.find_element(By.CSS_SELECTOR, ".field-status .readonly")
            status_text = status_field.get_attribute("textContent").strip()
            self.assertIn("success", status_text)
            log = self.find_element(By.CSS_SELECTOR, ".field-log .readonly").text
